...
proc chatwidget::Configure {self args} {
    ...
    switch -exact -- [set option [lindex $args 0]] {
        -chatstate {
            if {[llength $args] > 1} { set state(chatstate) [Pop args 1] } else { return $state(chatstate) }
        }
        -history {
            if {[llength $args] > 1} { set state(history) [Pop args 1] } else { return $state(history) }
        }
        -font {
            if {[llength $args] > 1} {
                set font [Pop args 1]
...
proc chatwidget::Message {self text args} {
    ...
    set idx [$chat search -exact $nick end-2l]
    if { [scan $idx %d.%d l r] != -1 } {
    set r [expr $r + [string length $nick]]
    $chat tag add I_NICK-$nick $idx ${l}.${r}
    $chat tag bind I_NICK-$nick <1> [list [namespace current]::isnert_nick_l $state(entry_widget) $nick]
    $chat tag bind I_NICK-$nick <Any-Enter> [list $chat configure -cursor hand2]
    $chat tag bind I_NICK-$nick <Any-Leave> [list $chat configure -cursor xterm]
    }
...
proc chatwidget::isnert_nick_l {w nick} {
    $w insert end "${nick}, "
    focus $w
}
...
proc chatwidget::Create {self} {
    ...
    set inner [ttk::panedwindow $outer.inner -orient horizontal -width 100]
    ...
    $names.text configure -width 40 -height 10 -state disabled \
    ...
    bind $names.text <Map> [list [namespace origin PaneMap] %W $inner -150]
    bind $entry.text <Map> [list [namespace origin PaneMap] %W $outer -56]
...
proc chatwidget::Nickcomplete {self} {
    ...
    if {[info exists state(nickcompletion)]} {
        ...
        set matches [lsearch -all -nocase -inline -glob -index 0 $state(names) $partial*]
        set index 0
    }
